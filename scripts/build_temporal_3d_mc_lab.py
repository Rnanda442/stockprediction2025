...
DEFAULT_PROFILES = "research_context/ticker_profiles_nasdaq_v1.json"
SECTOR_PALETTE = {'Technology': '#40c9c0', 'Health Care': '#f08a5d', 'Finance': '#e9b85a', 'Consumer Discretionary': '#dc6f91', 'Consumer Staples': '#8ec5b0', 'Industrials': '#7aa6d8', 'Energy': '#e06c4f', 'Utilities': '#90a955', 'Real Estate': '#bd8cdb', 'Telecommunications': '#5ca4a9', 'Basic Materials': '#c59b6d', 'Miscellaneous': '#a7b0a8', 'ETF / Fund': '#c8c4b7', 'Unclassified': '#7f8c8d'}
VIEW_PRESETS = [{'plot_id': 'marketPulsePlot', 'encoding_id': 'marketPulseEncoding', 'title': 'Market pulse', 'x': 'ret_5d', 'y': 'ret_20d', 'z': 'drawdown_60d', 'size': 'dollar_vol_20d_log', 'color': 'sector', 'color_kind': 'sector'}, {'plot_id': 'similarityPlot', 'encoding_id': 'similarityEncoding', 'title': 'Similarity space', 'x': 'latent_x', 'y': 'latent_y', 'z': 'latent_z', 'size': 'graph_degree', 'color': 'graph_similarity_mean', 'color_kind': 'continuous'}, {'plot_id': 'motionPlot', 'encoding_id': 'motionEncoding', 'title': 'Motion and regime', 'x': 'latent_velocity', 'y': 'latent_radial_expansion', 'z': 'graph_regime_residence_days', 'size': 'graph_cluster_switch_count_20d', 'color': 'crowding_change_5d', 'color_kind': 'continuous'}]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--security-profiles", default=DEFAULT_PROFILES)
...
def load_security_profiles(path: Path, tickers: list[str]) -> dict[str, dict[str, str]]:
    document = json.loads(path.read_text(encoding="utf-8"))
    securities = document.get("securities", document)
    fallback = {"name": "Unknown security", "sector": "Unclassified", "industry": "Not classified", "country": "Not listed"}
    return {ticker: {**fallback, **securities.get(ticker, {}), "name": securities.get(ticker, {}).get("name", ticker)} for ticker in tickers}
...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box} body{margin:0;color:var(--ink);font-family:"Avenir Next","Trebuchet MS",sans-serif;background:radial-gradient(circle at 8% 4%,rgba(233,184,90,.3),transparent 28%),radial-gradient(circle at 92% 16%,rgba(142,197,176,.28),transparent 30%),linear-gradient(145deg,#efe5d2,#f8f3e8 55%,#e6efe8);min-height:100vh}
    ...
    .selected-stock{margin-top:14px;display:grid;grid-template-columns:minmax(220px,.7fr) 1.3fr;gap:14px;padding:15px 18px;border-radius:18px;background:linear-gradient(120deg,rgba(8,47,50,.97),rgba(13,74,74,.9));color:#f4eddf;border:1px solid rgba(233,184,90,.22)}
    .selected-stock strong{display:block;font-family:Georgia,"Times New Roman",serif;font-size:20px}.selected-stock span{display:block;margin-top:4px;color:rgba(244,237,223,.72);font-size:12px}.selected-values{display:grid;grid-template-columns:repeat(4,minmax(90px,1fr));gap:8px}.selected-value{padding:8px 10px;border-radius:12px;background:rgba(255,255,255,.07)}.selected-value b{display:block;color:#e9b85a;font:700 10px/1.2 "IBM Plex Mono",monospace;text-transform:uppercase;letter-spacing:.08em}.selected-value em{display:block;margin-top:5px;font-style:normal;font-weight:700}
    .view-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px;padding:0 18px 20px}.view-card{min-width:0;overflow:hidden;border-radius:20px;background:linear-gradient(145deg,#0b393b,#082f32);border:1px solid rgba(233,184,90,.16);box-shadow:inset 0 1px 0 rgba(255,255,255,.05)}.view-head{display:flex;justify-content:space-between;gap:14px;padding:14px 16px 0;color:#f4eddf}.view-head h3{margin:0;font:700 19px/1.1 Georgia,"Times New Roman",serif}.encoding{max-width:62%;text-align:right;color:rgba(244,237,223,.66);font:600 9px/1.45 "IBM Plex Mono",monospace;text-transform:uppercase;letter-spacing:.04em}.view-plot{height:390px}.sector-legend{display:flex;flex-wrap:wrap;gap:6px;padding:0 14px 14px}.sector-chip{display:inline-flex;align-items:center;gap:5px;color:rgba(244,237,223,.78);font-size:9px}.sector-chip i{width:8px;height:8px;border-radius:50%;display:inline-block}.view-note{padding:0 16px 13px;color:rgba(244,237,223,.58);font-size:10px;line-height:1.45}
    @media(max-width:1180px){.view-grid{grid-template-columns:1fr}.selected-stock{grid-template-columns:1fr}.selected-values{grid-template-columns:repeat(2,minmax(100px,1fr))}}@media(max-width:680px){.view-grid{padding:0 8px 12px}.view-plot{height:340px}.view-head{display:block}.encoding{max-width:none;text-align:left;margin-top:7px}.selected-values{grid-template-columns:1fr 1fr}}
...
<main class="shell">
  ...
  <section class="control-deck">
    ...
    <div class="timeline"><button id="playButton">Play history</button><input id="dateSlider" type="range" min="0" step="1"><span id="dateReadout" class="date-readout"></span><label class="check"><input id="trailToggle" type="checkbox" checked> selected-stock trail</label></div><div id="selectedStockSummary" class="selected-stock"></div>
  </section>

  <section class="main-grid">
    <article class="panel"><header class="panel-head"><div><div class="micro">Coordinated 3D atlas</div><h2>Four views of the same stock universe</h2><p>Every sphere is the same stock and date across panels. Click a sphere to identify the company and carry its trail through every view.</p></div><div id="universeCount" class="micro"></div></header><div class="view-grid"><section class="view-card"><div class="view-head"><h3>Market pulse</h3><div id="marketPulseEncoding" class="encoding"></div></div><div id="marketPulsePlot" class="view-plot"></div><div id="sectorLegend" class="sector-legend"></div></section><section class="view-card"><div class="view-head"><h3>Similarity space</h3><div id="similarityEncoding" class="encoding"></div></div><div id="similarityPlot" class="view-plot"></div><p class="view-note">Closeness is descriptive. It is not proof of shared future returns.</p></section><section class="view-card"><div class="view-head"><h3>Motion and regime</h3><div id="motionEncoding" class="encoding"></div></div><div id="motionPlot" class="view-plot"></div><p class="view-note">Fast motion or crowding is a hypothesis flag, not a trading signal.</p></section><section class="view-card"><div class="view-head"><h3>Custom research view</h3><div id="customEncoding" class="encoding"></div></div><div id="customPlot" class="view-plot"></div><p class="view-note">Use the controls above to remap coordinates, sphere size, and color.</p></section></div></article>
...
const profiles=DATA.security_profiles||{};
const sectorPalette=DATA.sector_palette||{};
const presetViews=DATA.view_presets||[];
...
function profile(ticker){return profiles[ticker]||{name:ticker,sector:"Unclassified",industry:"Not classified",country:"Not listed"}}
function escapeHtml(value){return String(value??"").replace(/[&<>"']/g,char=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[char]))}
function formatValue(key,value){if(value==null||!Number.isFinite(Number(value)))return "-";const numeric=Number(value);if(/ret_|vol_|drawdown|probability|expansion|divergence|convergence/.test(key))return formatPercent(numeric);if(/degree|days|switch|curvature/.test(key))return numeric.toFixed(1);return numeric.toFixed(3)}
function encodingText(view){const color=view.color_kind==="sector"?"sector":axisTitle(view.color);return `X ${axisTitle(view.x)} / Y ${axisTitle(view.y)} / Z ${axisTitle(view.z)} / Size ${axisTitle(view.size)} / Color ${color}`}
function hoverText(frame,index,view){const ticker=frame.tickers[index],security=profile(ticker),value=key=>frame.values[key]?.[index];return `<b>${escapeHtml(ticker)} | ${escapeHtml(security.name)}</b><br>${escapeHtml(security.sector)} / ${escapeHtml(security.industry)}<br>${frame.date}<br>X - ${axisTitle(view.x)}: ${formatValue(view.x,value(view.x))}<br>Y - ${axisTitle(view.y)}: ${formatValue(view.y,value(view.y))}<br>Z - ${axisTitle(view.z)}: ${formatValue(view.z,value(view.z))}<br>Size - ${axisTitle(view.size)}: ${formatValue(view.size,value(view.size))}<br>Color - ${view.color_kind==="sector"?escapeHtml(security.sector):`${axisTitle(view.color)}: ${formatValue(view.color,value(view.color))}`}`}
function currentViews(){return [...presetViews,{plot_id:"customPlot",encoding_id:"customEncoding",title:"Custom research view",x:byId("xSelect").value,y:byId("ySelect").value,z:byId("zSelect").value,size:byId("sizeSelect").value,color:byId("colorSelect").value,color_kind:"continuous"}]}
function drawSectorLegend(frame){const sectors=[...new Set(frame.tickers.map(ticker=>profile(ticker).sector))].sort();byId("sectorLegend").innerHTML=sectors.map(sector=>`<span class="sector-chip"><i style="background:${sectorPalette[sector]||sectorPalette.Unclassified}"></i>${escapeHtml(sector)}</span>`).join("")}
function drawSelectedSummary(frame){const index=Math.max(0,frame.tickers.indexOf(selectedTicker)),ticker=frame.tickers[index],security=profile(ticker),value=key=>frame.values[key]?.[index];byId("selectedStockSummary").innerHTML=`<div><strong>${escapeHtml(ticker)} | ${escapeHtml(security.name)}</strong><span>${escapeHtml(security.sector)} / ${escapeHtml(security.industry)} / ${escapeHtml(security.country)} / ${frame.date}</span></div><div class="selected-values"><div class="selected-value"><b>20d momentum</b><em>${formatValue("ret_20d",value("ret_20d"))}</em></div><div class="selected-value"><b>60d volatility</b><em>${formatValue("vol_60d",value("vol_60d"))}</em></div><div class="selected-value"><b>Similarity</b><em>${formatValue("graph_similarity_mean",value("graph_similarity_mean"))}</em></div><div class="selected-value"><b>Latent speed</b><em>${formatValue("latent_velocity",value("latent_velocity"))}</em></div></div>`}
function drawView(frame,view){const sectorMode=view.color_kind==="sector",colorValues=sectorMode?frame.tickers.map(ticker=>sectorPalette[profile(ticker).sector]||sectorPalette.Unclassified):frame.values[view.color];const marker={size:sizes(frame.values[view.size]),color:colorValues,opacity:.82,line:{color:"rgba(255,255,255,.55)",width:.5}};if(!sectorMode){marker.colorscale=[[0,"#c95c3f"],[.48,"#e9b85a"],[.52,"#dce8d7"],[1,"#0d736c"]];marker.showscale=true;marker.colorbar={title:axisTitle(view.color),thickness:9,len:.48,x:1.01}}
const traces=[{type:"scatter3d",mode:"markers",x:frame.values[view.x],y:frame.values[view.y],z:frame.values[view.z],customdata:frame.tickers,text:frame.tickers.map((_,i)=>hoverText(frame,i,view)),hovertemplate:"%{text}<extra></extra>",marker}];const selectedIndex=frame.tickers.indexOf(selectedTicker);if(selectedIndex>=0){traces.push({type:"scatter3d",mode:"markers+text",x:[frame.values[view.x][selectedIndex]],y:[frame.values[view.y][selectedIndex]],z:[frame.values[view.z][selectedIndex]],customdata:[selectedTicker],text:[selectedTicker],textposition:"top center",textfont:{color:"#fff4d2",size:11},hoverinfo:"skip",marker:{size:12,color:"#fff4d2",symbol:"diamond",line:{color:"#d6753f",width:4}}})}
if(selectedTicker&&byId("trailToggle").checked){const trail={x:[],y:[],z:[],text:[]};for(let i=Math.max(0,frameIndex-20);i<=frameIndex;i++){const past=DATA.frames[i],position=past.tickers.indexOf(selectedTicker);if(position>=0){trail.x.push(past.values[view.x][position]);trail.y.push(past.values[view.y][position]);trail.z.push(past.values[view.z][position]);trail.text.push(past.date)}}traces.push({type:"scatter3d",mode:"lines",x:trail.x,y:trail.y,z:trail.z,text:trail.text,hovertemplate:`<b>${selectedTicker}</b><br>%{text}<extra></extra>`,line:{color:"#fff4d2",width:6}})}
const layout={margin:{l:0,r:12,t:4,b:0},paper_bgcolor:"rgba(0,0,0,0)",plot_bgcolor:"rgba(0,0,0,0)",showlegend:false,scene:{bgcolor:"#082f32",xaxis:{title:axisTitle(view.x),gridcolor:"rgba(255,255,255,.12)",color:"#dce8df"},yaxis:{title:axisTitle(view.y),gridcolor:"rgba(255,255,255,.12)",color:"#dce8df"},zaxis:{title:axisTitle(view.z),gridcolor:"rgba(255,255,255,.12)",color:"#dce8df"},camera:{eye:{x:1.38,y:1.35,z:.9}}}};Plotly.react(view.plot_id,traces,layout,{responsive:true,displaylogo:false});byId(view.encoding_id).textContent=encodingText(view);const plot=byId(view.plot_id);if(!plot.dataset.bound){plot.on("plotly_click",event=>{const ticker=event.points[0]?.customdata;if(typeof ticker==="string"){selectedTicker=ticker;byId("tickerSearch").value=ticker;drawUniverse();drawMonteCarlo()}});plot.dataset.bound="1"}}
function drawUniverse(){const frame=DATA.frames[frameIndex];for(const view of currentViews())drawView(frame,view);drawSectorLegend(frame);drawSelectedSummary(frame);byId("dateReadout").textContent=frame.date;byId("universeCount").textContent=`${frame.tickers.length} stocks / frame ${frameIndex+1} / four linked views`}
...
def main() -> None:
    ...
    security_profiles = load_security_profiles(Path(args.security_profiles), tickers)
